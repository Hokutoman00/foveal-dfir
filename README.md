...
4. **Divergence as the primary signal** *(in progress)*. The investigator and the blind grader produce three explicit states — `AGREE_REAL`, `AGREE_FP`, `DISAGREE` — and `DISAGREE` is the escalate set. Inter-observer disagreement is treated as evidence of where the truth lives, not noise to reconcile.
5. **Boundary register: declared blind spots** *(in progress)*. Areas the agent did *not* examine, or where it could not resolve to a verdict, are emitted as a first-class output. Missed evidence is reported as missed, never silently dropped.
6. **Actor-cadence analysis (agent-vs-agent)** *(in progress)*. The hackathon's stated motivation is Anthropic's GTG-1002: autonomous AI attackers. We model the adversary as an agent and exploit a structural property — machine-paced timing — to distinguish a likely autonomous-AI actor from a human one and direct attention accordingly.
7. **Pre-registered falsifiers** *(in progress)*. For every "evil" hypothesis, the killer evidence is **declared in advance** and actively hunted. Anti-tunnel-vision made Popperian and structural.
8. **Responsibility ledger** *(in progress)*. Each claim records which observer produced it, where observers diverged, and which entity carries each verdict. Distributed contribution, traceable accountability — not diffuse responsibility.
